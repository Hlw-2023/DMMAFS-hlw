...
def conv_norm_lrelu(input_dim, output_dim, kernel_size=(3,3),padding=(1,1), stride=(1,1)):
    conv = Conv2dBlock(input_dim,output_dim,3,1,1,norm="batch",activation="gelu",pad_type="reflect")
    layer = [conv]
...
class ResBlock(nn.Module):
    def __init__(self, dim, norm="batch", activation="lrelu", pad_type="zero"):
...
class Conv2dBlock(nn.Module):
    def __init__(
        ...
        if group == "none":
            ...
            self.conv = nn.Conv2d(input_dim, output_dim, kernel_size, stride, bias=self.use_bias,groups=int(group))
    def forward(self, x):
